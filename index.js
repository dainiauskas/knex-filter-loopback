...
module.exports = function (knex, translationTable) {
  ...
  var handlers = {
    ...
    'regexp' : _.partial(comparisonHandler, 'regexp'),
  ...
  function translateField(field) {
    if (translationTable && translationTable[field]) {
      return knex.raw(translationTable[field]);
    }
  ...
  function walkLevel (level) {
    ...
    _.each(compactObject(level), function (value, key) {
      ...
      handlers[command.command].call(builder, translateField.call(builder, command.field), command.val);
    });
  }
